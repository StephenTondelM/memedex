...
class MemePostControllerTest {
    ...
    void beforeEach() {
        ...
        invalidMemePostFake = MemePost.builder()
                .user("sm")
                .memeUrl("invalid").build();
    }
    ...
    void getAllValidRequestWithAllFieldsAndDefaultPaginationAndDescOrderByTimeTest() throws Exception {
        ...
        mockMvc.perform(get("/api/memepost"))
                ...
                .andExpect(jsonPath("$.content.[0].memeUrl").value("http://memesareus.com"))
                .andExpect(jsonPath("$.content.[0].timestamp").value(1235123))
                .andExpect(jsonPath("$.content.[1].id").value(("2")))
                .andExpect(jsonPath("$.content.[1].user").value("testUser2"))
                .andExpect(jsonPath("$.content.[1].memeUrl").value("http://memesareus.com"))
    ...
    void getByIdValidRequestWithExistingResourceAndAllFieldsTest() throws Exception {
        ...
        mockMvc.perform(get("/api/memepost/1"))
                ...
                .andExpect(jsonPath("$.memeUrl").value("http://memesareus.com"))
    ...
    @Test
    void createPostInvalidRequestWithInvalidPostTest() throws Exception {
        mockMvc.perform(post("/api/memepost")
                        .content(asJsonString(invalidMemePostFake))
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isBadRequest())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                // TODO fieldErrors are not in order making it difficult to test. jsonPath should be able to use filter predicate.
//                .andExpect(jsonPath("$.fieldErrors[0][0]").value("memeUrl"))
//                .andExpect(jsonPath("$.fieldErrors[0][1]").value("Please put a valid URL"))
//                .andExpect(jsonPath("$.fieldErrors[1][0]").value("user"))
//                .andExpect(jsonPath("$.fieldErrors[1][1]").value("Username needs to be between 3 and 30 characters"))
                .andExpect(jsonPath("$.status").value(400));

        verify(memePostRepositoryMock, times(0)).findById(any(String.class));
        verify(memePostRepositoryMock, times(0)).save(any(MemePost.class));
        verify(memeSocketControllerMock, times(0)).sendNewMeme(any(MemePost.class));
    }
    ...
    @Test
    void updateByIdInvalidRequestWithInvalidPostTest() throws Exception {
        mockMvc.perform(put("/api/memepost/12")
                        .content(asJsonString(invalidMemePostFake))
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
